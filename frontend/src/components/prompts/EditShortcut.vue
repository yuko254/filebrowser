<template>
  <div class="card floating small-edit-shortcut">
    <div class="card-content">
      ...
      <input v-model="name" id="focus-prompt" />
      <p style="margin-top:8px; font-size:0.9em; color:var(--muted)">{{ path }}</p>
    </div>
    ...
    async doSave() {
      try {
        ...
        if (this.idx >= 0 && this.idx < shortcuts.length) {
          ...
          await usersApi.update({ id: auth.user?.id, shortcuts }, ['shortcuts']);
...
<style scoped>
.small-edit-shortcut { width: 320px; }
.small-edit-shortcut .card-content input { width: 100%; }
</style>
